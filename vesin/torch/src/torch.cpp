...
std::vector<torch::Tensor> NeighborListHolder::compute(
    ...
    torch::optional<torch::Tensor> shifts = torch::nullopt;
    ...
    torch::optional<torch::Tensor> distances = torch::nullopt;
    ...
    torch::optional<torch::Tensor> vectors = torch::nullopt;
    ...
    // handle autograd
    auto distances_tensor = torch::Tensor();
    auto vector_tensor = torch::Tensor();
    if (return_distances || return_vectors) {
        auto outputs = AutogradNeighbors::apply(points, box, periodic, pairs, shifts, distances, vectors);
        distances_tensor = outputs[0];
        vector_tensor = outputs[1];
    }
    ...
    for (auto c: quantities) {
        if (c == 'i') {
            ...
            output.push_back(shifts.value());
        } else if (c == 'd') {
            output.push_back(distances_tensor);
        } else if (c == 'D') {
            output.push_back(vector_tensor);
        } else {
            C10_THROW_ERROR(ValueError,
...
TORCH_LIBRARY(vesin, m) {
    ...
    m.class_<NeighborListHolder>("NeighborList")
        .def(
...
std::vector<torch::Tensor> AutogradNeighbors::backward(
    ...
    auto vectors_grad = outputs_grad[1];
