...
static VesinDevice torch_to_vesin_device(torch::Device device) {
  if (device.is_cpu()) {
    ...
    throw std::runtime_error("device " + device.str() +
                             " is not supported in vesin");
  }
}
...
public:
  ...
  forward(torch::autograd::AutogradContext *ctx, torch::Tensor points,
          torch::Tensor box, bool periodic, torch::Tensor pairs,
...
NeighborListHolder::NeighborListHolder(double cutoff, bool full_list,
                                       bool sorted)
    : cutoff_(cutoff), full_list_(full_list), sorted_(sorted), data_(nullptr) {
...
NeighborListHolder::compute(torch::Tensor points, torch::Tensor box,
                            bool periodic, std::string quantities, bool copy) {
  ...
  if (points.scalar_type() != box.scalar_type()) {
    C10_THROW_ERROR(
        ValueError,
        std::string(
            "expected `points` and `box` to have the same dtype, got ") +
  ...
  auto status = vesin_neighbors(
      reinterpret_cast<const double(*)[3]>(points.data_ptr<double>()), n_points,
      reinterpret_cast<const double(*)[3]>(box.data_ptr<double>()), periodic,
      device, options, data_, &error_message);

  if (status != EXIT_SUCCESS) {
    throw std::runtime_error(std::string("failed to compute neighbors: ") +
                             error_message);
  }
  ...
  if ((return_distances || return_vectors)) {
    ...
    auto outputs =
        AutogradNeighbors::apply(points, box, periodic, pairs, shifts_optional,
                                 distances_optional, vectors_optional);
  ...
  for (auto c : quantities) {
    if (c == 'i') {
      ...
      C10_THROW_ERROR(ValueError, "unexpected character in `quantities`: " +
                                      std::string(1, c));
    }
  }
...
TORCH_LIBRARY(vesin, m) {
  ...
  m.class_<NeighborListHolder>("_NeighborList")
      .def(torch::init<double, bool, bool>(), DOCSTRING,
           {torch::arg("cutoff"), torch::arg("full_list"),
            torch::arg("sorted") = false})
      .def("compute", &NeighborListHolder::compute, DOCSTRING,
           {torch::arg("points"), torch::arg("box"), torch::arg("periodic"),
            torch::arg("quantities"), torch::arg("copy") = true});
}
...
std::vector<torch::Tensor>
AutogradNeighbors::forward(torch::autograd::AutogradContext *ctx,
                           torch::Tensor points, torch::Tensor box,
                           bool periodic, torch::Tensor pairs,
                           torch::optional<torch::Tensor> shifts,
                           torch::optional<torch::Tensor> distances,
                           torch::optional<torch::Tensor> vectors) {
...
std::vector<torch::Tensor>
AutogradNeighbors::backward(torch::autograd::AutogradContext *ctx,
                            std::vector<torch::Tensor> outputs_grad) {
  ...
  if (points.requires_grad() || box.requires_grad()) {
    ...
    vectors_grad +=
        distances_grad.index(
            {torch::indexing::Slice(), torch::indexing::None}) *
        vectors /
        distances.index({torch::indexing::Slice(), torch::indexing::None});
  }
  ...
  return {
      points_grad,     box_grad,        torch::Tensor(), torch::Tensor(),
      torch::Tensor(), torch::Tensor(), torch::Tensor(),
  };
}
