...
module vesin_cdef
    ...
    type, bind(c) :: VesinOptions
        ...
        !< Should the returned `NeighborList` contain `shifts`?

        logical(c_bool) :: return_distances = .false.
        !< Should the returned `NeighborList` contain `distances`?

        logical(c_bool) :: return_vectors = .false.
        !< Should the returned `NeighborList` contain `vector`?
    ...
    !! @param error_message a `type(c_ptr)` to a null-terminated `char*`
    !!     containing the error message of this function. Cast to:
    !!~~~~~{.f90}
