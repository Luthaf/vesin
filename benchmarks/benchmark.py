...
def setup_vesin_cpu(quantities, atoms, cutoff):
    points = np.asarray(atoms.positions)
    box = np.asarray(atoms.cell[:])
    periodic = np.asarray(atoms.pbc)

    return cutoff, quantities, points, box, periodic


def setup_vesin_cuda(quantities, atoms, cutoff):
    import cupy as cp
...
def vesin_run(cutoff, quantities, points, box, periodic):
    import vesin
